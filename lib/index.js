...
function Client(options) {
  ...
  this.useNoCacheRegex = options.useNoCacheRegex || false
...
Client.prototype.uploadFile = function(params) {
  ...
  if (self.useNoCacheRegex) {
    // if it is a file with a version hash from astro use the outside cache control, otherwise overwrite with no-cache 
    if (!/^.+\.[0-9a-f]{9,9}\..+$/.test(localFile)) {
